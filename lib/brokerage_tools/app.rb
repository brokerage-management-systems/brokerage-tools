...
module BrokerageTools
  class App
    ...
    def self.parse
      lib = File.join('brokerage_tools', "#{$options.parser_type}", "#{$options.parser_type}_parser")
      require lib
      ...
      Dir.entries(directory).each do |file|
        if $options.zips && File.extname(file.downcase) == '.zip'
          ...
          zip_file.each do |entry|
            ...
            if File.exists? extracted_file_name_with_path
              File.delete extracted_file_name_with_path
            end
            zip_file.extract(entry, extracted_file_name_with_path)
            parse_file(parser, directory, entry.to_s)
            if File.file?(extracted_file_name_with_path) && File.exists?(extracted_file_name_with_path)
              File.delete extracted_file_name_with_path
            end
          end
        else
